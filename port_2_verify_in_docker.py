...
MOUNTED_DIR = "/app"
FILE_MAP_TXT = os.path.join(MOUNTED_DIR, "./file_map.txt")
VERIFY_RESULT_TXT = os.path.join(MOUNTED_DIR, "./verify_result.txt")
...
def get_tests():
    ...
    with open(FILE_MAP_TXT, "r") as f:
        for line in f:
            ...
            test_parameters[test] = [conf for conf in config_list if "=" in conf]
...
def write_verify_result(verify_result):
    ...
    with open(VERIFY_RESULT_TXT, "w") as f:
        for test in verify_result:
            original_test_name = test.strip(MOUNTED_DIR)
